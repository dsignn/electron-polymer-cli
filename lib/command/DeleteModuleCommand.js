 ...
 *
 */
class DeleteModuleCommand extends AbstractCommand_1.AbstractCommand {
    constructor() {
        super(...arguments);
    ...
    action(nameModule) {
        ...
        if (this.validatorContainer.get('ModuleNameValidation').isValid(nameModule)) {
            this.errorMessage(`Invalid name accept fdfds "${AbstractCommand_1.AbstractCommand.REGEX_NAME_MODULE}" given "${nameModule}"\n`);
        }
        /**
